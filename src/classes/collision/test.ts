...
export class SATCollisionDetector {
    detectCollision(shapeA: Shape, shapeB: Shape): CollisionInfo | null {
        // Get all axes to test from both shapes
        ...
        for (const axis of axes) {
            ...
            // If no overlap on any axis, shapes are not colliding
            if (overlap === null) {
                return null;
            }
        ...
        // If we get here, shapes are colliding
        // Find the contact point using the minimum overlap axis
    ...
    private findContactPoint(shapeA: Shape, shapeB: Shape, axis: Vector): Vector {
        // This will be implemented based on the specific shape types
